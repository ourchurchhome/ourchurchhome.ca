/**
 * src/cms/components/ImageLibrary.tsx
 *
 * A modal dialog for browsing, uploading, and selecting images.
 * Used by the Image field control and the VisualEditor toolbar.
 */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800 shrink-0">
          <h2 className="text-sm font-semibold text-white">Image Library</h2>
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              disabled={uploading}
              className="px-3 py-1 text-xs bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white rounded-md transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {error && <p className="mb-3 text-xs text-red-400 bg-red-900/30 border border-red-700 rounded px-3 py-2">{error}</p>}
          {loading ? (
            <p className="text-sm text-gray-500 text-center py-12">Loading…</p>
          ) : images.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-12">No images yet. Upload one to get started.</p>
          ) : (
              ...
              {images.map((img) => (
                <div key={img.path} className="group relative flex flex-col gap-1">
                  <button
                    ...
                    className="block w-full aspect-square rounded-md overflow-hidden border border-gray-700 hover:border-blue-500 transition-colors bg-gray-800"
                  >
                    <img src={img.url} alt={img.filename} className="w-full h-full object-cover" loading="lazy" />
                  </button>
                  <p className="text-xs text-gray-500 truncate text-center px-0.5" title={img.filename}>{img.filename}</p>
                  {img.sizeBytes > 0 && <p className="text-xs text-gray-600 text-center">{fmtSize(img.sizeBytes)}</p>}
                  <button
                    type="button"
